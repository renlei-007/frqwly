<template>
	<view class="content myactive">
		<ys-scroll :param="param" ref="scroll" @refresh="refresh" @loadMore = "loadMore" v-show="activeList.length>0">
			<view class="data_box">
				<view class="data_list" v-for="(item,index) in activeList" :key="index" @tap="toDetail(item.id)">
					...
					<view class="data_list_info">
						...
						<view class="data_list_info_text">活动时间:{{item.bookingDate}} {{item.slot}}</view>
						<view class="data_list_info_text">预约时间:{{item.createTime}}</view>
						<view class="data_list_info_text">活动地址:{{item.content.attr_address}}</view>
					</view>
				</view>
			</view>
		</ys-scroll>
		<view class="no_data" v-show="activeList.length==0">

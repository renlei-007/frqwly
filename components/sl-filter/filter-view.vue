					...
					<view class="filter-content-list">
						<view v-for="(detailItem,idx) in selectDetailList" :key="idx" :class="detailItem.isSelected?'filter-content-list-item-active':'filter-content-list-item-default'"
						 :style="{'color': detailItem.isSelected?themeColor:'#666666'}" @tap="sortTap(idx,selectDetailList,item.key)">
							<text class="uni-ellipsis">{{detailItem.title}}</text>
						</view>
					</view>
				</view>
				<view v-else>
...
<style>
	...
	.filter-content-list-item-default {
		color: #666666;
		width: 50%;
		padding: 0px 0px;float:left; text-align: center;margin: 5upx 0 ;
	}

	.filter-content-list-item-default text {
		width: 90%;
		font-size: 14px;
		display: inline-block;border:#eee solid 1px; background: #fff; padding: 10upx 0;border-radius: 3px;
	}
	...
	.filter-content-list-item-active text {
		font-size: 14px;
		width: 90%;
		display: inline-block;border:#dd7070 solid 1px; background: #fff;padding: 10upx 0;border-radius: 3px;color: #9d1515;
	}

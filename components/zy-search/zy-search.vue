<template name="zy-search">
	<view id="zy-search">
		...
		<template v-if="isBlock">
			<view class="s-block" v-if="hisList.length > 0">
				<view class="header">
					历史记录
					<image src="../../static/zy-search/delete.svg" mode="aspectFit" @click="delhistory"></image>
				</view>
				<view class="list">
					<view v-for="(item,index) in hisList" :key="index" @click="keywordsClick(item)">{{item}}</view>
				</view>
			</view>
		...
		<template v-else>
			<view class="s-circle" v-if="hisList.length > 0">
				<view class="header">
					历史记录
					<image src="../../static/zy-search/delete.svg" mode="aspectFit" @click="delhistory"></image>
				</view>
...
<style lang="less" scoped>
	.searchs{
		...
		input{
			...
			padding: 10upx 74upx 10upx 30upx;
			font-size: 28upx;
			border-radius: 50upx;
		}
		.voice-icon{
